...
<details open>
  ...
  <p>Coroutines in Lua are not operating system threads or processes. Coroutines are blocks of Lua code which are created within Lua and have their own flow of control like threads. Only one coroutine runs at a time, and it runs until it activates another coroutine or yields (returns to the coroutine that invoked it). Coroutines are a way to express multiple cooperating threads of control in a convenient and natural way, but do not execute in parallel, and thus gain no performance benefit from multiple CPU's. However, since coroutines switch much faster than operating system threads and do not typically require complex and sometimes expensive locking mechanisms, using coroutines is typically faster than the equivalent program using full OS threads.</p>
</details>

<details open>
  <summary>Yielding</summary>
  <p>In order for multiple coroutines to share execution they must stop executing (after performing a sensible amount of processing) and pass control to another ba.thread. This act of submission is called yielding. Coroutines explicitly call a Lua function coroutine.yield(), which is similar to using return in functions. What differentiates yielding from function returns is that at a later point we can reenter the thread and carry on where we left off. When you exit a function scope using return, the scope is destroyed and we cannot reenter it.</p>
...
<details open>
  ...
  <p>We can find out what state the thread is in using the coroutine.status() function.</p>
  ...
  <p>The coroutine.resume() function returns the error status of the
...
<details open>
  ...
  <p>Event driven systems typically become complicated since a logically blocking sequence must be written in a state-machine style. A coroutine has its own stack and can yield at any location in the program. Event driven systems can, therefore, be greatly simplified when used in combination with coroutines.</p>
  ...
  <p>Click the above Run method. The following examples require that you have a timer instance. Please note that the timer events are printed to the server's console window so please keep the console window next to your browser window. Alternatively, open the <a target="_blank" href="/rtl/tracelogger/">TraceLogger</a> in a separate browser window.</p>
  <p>One can dynamically change the interval time by calling timer:reset.</p>
  <div class="lspeditor" extype="lua" example="10.11"></div>
  <p>Calling timer:set on a running timer has the same effect as calling timer:reset.</p>
  ...
  <p>You can restart a stopped timer with timer:set.</p>
  <div class="lspeditor" extype="lua" example="10.15"></div>
  <p>Removing the reference, the anchoring point, will eventually make the garbage collector remove an active interval timer. The timer should still print data in the server's console window since the garbage collector has so far not collected the timer object.</p>
...
<details open>
  ...
  <p>In the above examples, we used the server's console window when printing from the timer thread. We cannot use the print function in the LSP page since the LSP page's ephemeral request/response environment, which we covered in the <a href="Lua-Scope.lsp#server">scope tutorial</a>, has expired when the timer runs.</p>
  <div class="lspeditor" example="10.18"></div>
  <p>Click the above Run button and watch the printouts in the server's console window. Notice that you get a Lua exception when line 9 executes. You get the exception since you are using the print method in the LSP page's request/response environment. This ephemeral environment only exists as long as the LSP page is active and is long gone when the "timeout" timer function ends. Try adding "_G." in front of the print function on line 9 to force the use of the global print function, which prints to the server's console window and not to the response object. Click the Run button when you have fixed the code and watch the printouts in the console window. You should no longer get an exception.</p>
  <h4>Using the 'app' environment</h4>
