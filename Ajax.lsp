...
<details open>
  ...
  <p>AJAX is HTTP commands sent from the browser to the server using the browser's XMLHttpPRequest object. The XMLHttpPRequest object, which is accessible from JavaScript, is an API/object that enables JavaScript code to send HTTP commands directly to a server. Asynchronous means that the functions in the XMLHttpPRequest object do not block. Instead a JavaScript callback function is provided which is then called by the XMLHttpPRequest object when the server returns a response. Using callbacks (aka events) is common in JavaScript since the typical browser JavaScript environment is single threaded. A blocking call would cause the browser's user interface to freeze while waiting.</p>
  <p>The XMLHttpPRequest API is weird and cumbersome, but luckily many JavaScript libraries make it very easy to use this object. We will be using the very popular <a target="_blank" href="http://jquery.com/">jQuery JavaScript library</a> instead of directly using the XMLHttpPRequest. Although you can find many libraries that help you fast track your client side JavaScript development, we particularly like jQuery since it is small and makes it easy to write compact JavaScript code without having to learn the details of the browser's Document Object Model. JQuery is your perfect companion when it comes to writing your client side JavaScript code. The JQuery JavaScript library is also embedded in the Mako Server's resurce file (mako.zip). You can load this library from the server by navigating to <a target="_blank" href="/rtl/jquery.js">http://<?lsp=request:peername()?>/jquery.js</a>.</p>
</details>

<details open>
  <summary>jQuery Introduction</summary>
  <p>Although providing a comprehensive tutorial on JQuery is beyond the scope of this article, we will give you a quick introduction to the $() function since many get very confused by this JQuery library function. In JavaScript, $ is a valid character for variable names. The JQuery $() function is a Jack of all trades in JQuery and the function accepts a wide variety of arguments. For example, the following code will cause the browser to popup a "hello" message when the page has loaded.</p>
  <div class="lspeditor" example="7.1"></div>
  <p>In the above example, we pass in an anonymous function to the JQuery $() function at line 5.</p>
  <p>Let's go ahead and create a basic JQuery example that installs a keyboard event handler. Every time the event triggers, the keyboard key number is converted to text and the text is then appended to the HTML in the browser.<br/>
  <div class="lspeditor" example="7.2"></div>
  <p>Click the above Run button and start entering text in the input field.</p>
  <p>We install an "on page load" event callback function on line 5 by providing an anonymous function to the jQuery <b>$()</b> function. The "on page load" callback function will be called when the page has loaded and the Document Object Model (DOM) is ready. We start modifying the DOM on line 6 so it is important that the DOM is ready at that point.</p>
  <p>Line 14 to 16 is our static HTML. The message "Please enable JavaScript" should not be visible to users that have JavaScript enabled since the JavaScript code on line 5 erases the content within the &lt;h1&gt; element. The code construction $("#out").empty(); means look up the HTML element in the DOM tree whose ID is "out" and empty this object.</p>
  <p>The construction on line 7 means look up the HTML element in the DOM tree whose ID is "in" and install a keyboard event handler within the realm of the &lt;input id="in" type="text" /&gt; element. When a key is pressed in this input element, the keyboard event triggers, which in turn looks up the "out" element, converts the keyboard event to a character, and appends the character to the "out" element. The "out" element is the H1 tag on line 14.</p>
</details>

<details open>
  <!-- <h3>AJAX Example</h3> -->
  <summary>AJAX Example</summary>
  <p>As you have probably noticed, we have so far not used any AJAX. We will soon add AJAX to our example. It is not possible to use AJAX if you do not have an understanding of how to manipulate the browser's DOM since you cannot display the result from your AJAX response unless you know how to update the HTML by using JavaScript. The example below shows how to do that.</p>
  <p>We use the JavaScript function String.fromCharCode() in example 7.2 on line 8 above. This function converts the key code from the keyboard event to a string containing one character. Let’s go ahead and modify line 7 to 9 in example 7.2 to the following:</p>
  <pre class="code">$("#in").keypress(function(ev) {
     $.getJSON(window.location,{key:ev.which}, function(rsp) {
        $("#out").append(rsp.char);
  });</pre>
  <p>jQuery provides a number of functions that simplifies the use of the browser’s raw XMLHttpPRequest API. One of these functions is <a target="_blank" href="http://api.jquery.com/jquery.getjson/">$.getJSON()</a>, which sends URL encoded data to the server and expects JSON as the response data from the server. The first argument to this function is window.location which is the URL to the page itself. This means we will be sending the AJAX request back to the same LSP page on the server. The second argument is a JavaScript object containing the key-value pairs we want to send to the server. A JavaScript object is similar to a table in Lua. We only need one key/value pair in this example and that is the keyboard event number. This number will be converted to a string containing one character by Lua code on the server side and the response will be encoded as JSON. The last argument to $.getJSON is the "on server response" callback function. The ‘rsp’ argument is the decoded JSON response data from the server. The JavaScript object ‘rsp’ (from the decoded JSON) contains one property "char" which is the keyboard event number converted to a string by the server side Lua code.</p>
  <p>The complete AJAX transformation modification to example 7.2 is shown below.</p>
  <div class="lspeditor" example="7.3"></div>
  <p>Line two in the above example is strictly not needed in our example. It’s just an extra security measure that makes sure only AJAX requests can trigger the Lua code on line 3 to 14. jQuery adds the HTTP header "x-requested-with" and checking for this header is a convenient method for filtering out all requests that are not AJAX requests.</p>
  ...
  <?lsp end ?>
  <p>Line 13 is where the magic happens. The method  <a target="_blank" href="/ba/doc/?url=lua.html#response_json">response:json</a> takes a Lua table as argument, converts this to JSON, sends the JSON data to the client, and stops the script (the LSP page) – in other words, no code is executed on the server after line 13. Lua functions do not require parentheses if only one argument is supplied to the function. Line 13 can be changed to response:json{char=resp}. We dynamically create a Lua table and pass this table into method response:json(). This table includes one key/value pair, where ‘char’ is the key and the data in ‘resp’ is the value.</p>
  <p>Note that the server side LSP page returns the HTML starting on line 17 and ending on line 36 if the request is not AJAX and if the client did not send the "char" key/value pair. If you think about this, you will realize that the LSP code in example 7.3 can send both HTML and JSON responses to a client.</p>
</details>

<details open>
  <summary class="exth">AJAX Limitations</summary>
  <p>AJAX is great in solutions where the client is the one responsible for generating all the events, but how do you use AJAX if the server is the one generating events? The server has no way of telling the client that there is new data since an HTTP request is only open when a client sends a request to the server. One way to circumvent this problem is to use AJAX and constantly poll the server for new data. However, this is not an ideal solution since you get a delay in the event notification on the client due to the poll delay. You also create unnecessary network traffic, and the server side code must be designed such that it can buffer the event data until the client sends the poll request. Our online article <a target="_blank" href="https://realtimelogic.com/articles/The-scalability-problem-with-using-HTTPS-for-M2M">The scalability problem with using HTTPS</a> explains this problem in detail.</p>
  <p>Using WebSockets would fix the polling problem since the client and server maintain a persistent connection where the server can, at any time, send an event to the client. The <a href="WebSockets.lsp">next tutorial</a> provides an introduction to WebSockets.</p>
</details>

<details open>
  <summary id="AJAXoverWebSockets" class="exth">AJAX over WebSockets</summary>
  <p>The following example is a copy of the example included with our <a target="_blank" href="https://makoserver.net/articles/AJAX-over-WebSockets">AJAX over WebSockets Tutorial</a>.</p>
  <p>Since WebSockets lets us multiplex data on the same connection, we can easily implement an AJAX library on top of WebSockets and still use the same WebSocket connection for bi-directional real-time data transfer.</p>
  <p>The original  AJAX over WebSockets example has been modified to run on the Lua example engine, and all example code has been combined into one file. You may want to read the next tutorial, <a href="WebSockets.lsp">WebSockets Introduction</a>, prior to studying the following example. We also recommend reading our <a target="_blank" href="https://makoserver.net/articles/AJAX-over-WebSockets">AJAX over WebSockets Tutorial</a> which explains how this example works. The JavaScript code in the example below uses the new <a target="_blank" href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Using_promises">Promise API</a>.</p>
